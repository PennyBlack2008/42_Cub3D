...
/*   Updated: 2020/11/17 01:40:04 by jikang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int					calculate_save_opt(int argc, char **argv)
{
	int				save_opt;
	int				strlen;

	strlen = ft_strlen("--save");
	if (ft_strlen(argv[argc - 1]) > ft_strlen("--save"))
		strlen = ft_strlen(argv[argc - 1]);
	save_opt = (argc == 3 && (ft_strncmp(argv[argc - 1],
					"--save", strlen) == 0));
	return (save_opt);
}
...
int					main(int argc, char **argv)
{
	...
	if (argc == 3)
		save_opt = calculate_save_opt(argc, argv);
	if ((argc == 3 && !save_opt) || argc > 3 || argc < 2)
	{
		error_handler("argument error: ", 22);
		exit(1);
	}
	w.wall.length = 150;
